...
def dbdist_featuriser(traj, indices, save_to_disk=None) -> np.ndarray:
...
def dbdihed_featuriser(traj, indices, save_to_disk=None) -> np.ndarray:
...
def aloop_featuriser(traj, indices, save_to_disk=None) -> np.ndarray:
    ...
    ################################################
    top = traj.topology
    aloop_traj = traj.atom_slice(top.select(f'resid {indices['aloop_start']} to {indices['aloop_end']}'))
    distances = md.compute_contacts(aloop_traj, contacts='all', scheme='ca')[0]
    if save_to_disk is not None: np.save(save_to_disk, distances)
    return distances


def achelix_featuriser(traj, indices, save_to_disk=None) -> np.ndarray:
...
def aloopdssp_featuriser(traj, indices, save_to_disk=None) -> np.ndarray:
...
def achelixdssp_featuriser(traj, indices, save_to_disk=None) -> np.ndarray:
    ...
    #############################################################
    top = traj.topology
    achelix_traj = traj.atom_slice(top.select(f'resid {indices['aC_start']} to {indices['aC_end']}'))   
...
def achelixdihed_featuriser(traj, indices, save_to_disk=None) -> np.ndarray:
    ########## alpha-C helix secondary structure ################
