...
def plot_ev(ev, c_centers, traj_all, traj_weights, title, savedir, \
    ...
    ax1, contour1, cbar1 = plot_energy2d(energy2d(traj_all[:,0], traj_all[:,1], weights=traj_weights), ax=ax1, cbar_ax=fe_cax, contourf_kws=dict(cmap=ct_cmap, alpha=ct_a))
    ax2, contour1, cbar2 = plot_energy2d(energy2d(traj_all[:,2], traj_all[:,1], weights=traj_weights), ax=ax2, cbar=False, contourf_kws=dict(cmap=ct_cmap, alpha=ct_a))

    ax1_eg2 = ax1.scatter(c_centers[:,0], c_centers[:,1], s=ev_s, c=ev, marker=ev_marker, alpha=ev_a, cmap=ev_cmap, norm=divnorm)
    fig.colorbar(ax1_eg2, cax=ev1_cax, format='%.1f', orientation='horizontal')

    ax2_eg2 = ax2.scatter(c_centers[:,2], c_centers[:,1], s=ev_s, c=ev, marker=ev_marker, alpha=ev_a, cmap=ev_cmap, norm=divnorm)
    fig.colorbar(ax2_eg2, cax=ev2_cax, format='%.1f', orientation='horizontal')
    
    if ex:
        ax1.scatter(c_centers[min_id,0], c_centers[min_id,1], s=ex_s, c='k', marker='X', label='min')
        ax1.scatter(c_centers[max_id,0], c_centers[max_id,1], s=ex_s, c='k', marker='v', label='max')
        ax2.scatter(c_centers[min_id,2], c_centers[min_id,1], s=ex_s, c='k', marker='X', label='min')
        ax2.scatter(c_centers[max_id,2], c_centers[max_id,1], s=ex_s, c='k', marker='v', label='max')
        ax1.legend()
        ax2.legend()
...
def plot_fe(traj_x, traj_y, traj_weights, savedir, cmap='nipy_spectral', plot_c_centers=True, \
            c_centers=None, c_centers_s=3, c_centers_marker='.', c_centers_a=0.5, c_centers_c='black'):
    
    fig, ax = plt.subplots(figsize=(7, 6))
    ax, contour, cbar = plot_energy2d(energy2d(traj_x, traj_y, weights=traj_weights), ax=ax, contourf_kws=dict(cmap=cmap))
    if plot_c_centers: 
        ax.scatter(c_centers[:,0], c_centers[:,1], s=c_centers_s, c=c_centers_c, marker=c_centers_marker, alpha=c_centers_a)
...
def plot_pcca(state_assignment, c_centers, savedir, c_centers_s=6, cmap='gist_rainbow'):
    ...
    s = ax.scatter(c_centers[:,0], c_centers[:,1], 
