...
class MSMStudy():
    ...
    def __init__(self, study_name, ftrajs, mapping, wk_dir, model_dir=None, **kwargs):
        self._seed = 42
    ...
    def _bayesian_MSM(self):
        ...
        if not self.count_mod:
            raise ValueError('Transition count not found. Run transition count estimation first')
        self.msm_mod = BayesianMSM(reversible=True).fit_fetch(self.count_mod)
        '''
    ...
    def transform(self, data_ftraj):
        ...
        return ttraj, connected_d, disconnected_d, pcca_assignment
    ...
    def restore_models(self, model_dir=None):
        if model_dir is None:
            model_dir = self.wk_dir / 'models'

        if isinstance(model_dir, str):
            model_dir = Path(model_dir)

        if not model_dir.is_dir():
            raise ValueError('Models directory not found. Check the working directory')
        
        with open(model_dir / 'study_params.txt', 'r') as f:
            for line in f:
                key, val = line.split(':')
                if key.strip('_') in self._allowed_paramters:
                    self.__dict__[key] = int(val.strip('\n'))
        
        if (model_dir / 'tica_model').exists():
            print('Restoring TICA model')
            tica_mod = pm.coordinates.tica()
            self.tica_mod = tica_mod.load(model_dir/'tica_model')
        ...
        if (model_dir / 'kmeans_model').exists():
            print('Restoring Kmeans model')
            kmeans_mod = pm.coordinates.cluster_kmeans()
            self.kmeans_mod = kmeans_mod.load(model_dir/'kmeans_model')
